...
function getRequestTimeoutMs() {
  const raw = process.env.GHOST_ADMIN_API_TIMEOUT_MS;
  if (!raw) return DEFAULT_TIMEOUT_MS;
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error("GHOST_ADMIN_API_TIMEOUT_MS must be a positive number");
  }
  return parsed;
}
...
function requestJson({ method, url, headers, body, timeoutMs }) {
  return new Promise((resolve, reject) => {
    ...
    if (timeoutMs) {
      // Prevent hung invocations if Ghost is unreachable.
      req.setTimeout(timeoutMs, () => {
        req.destroy(new Error(`Ghost API request timed out after ${timeoutMs}ms`));
      });
    }
...
async function fetchById({ type, id }) {
  ...
  const response = await ghostRequest({
    method: "GET",
    path: `${contentType}/${id}/`,
    query: { formats: "html", fields: "id,slug,title,html,updated_at" },
  });
...
function replaceTablesHtml({ html, tableHtmlById }) {
  const entries = normalizeTableEntries(tableHtmlById);
  if (entries.length === 0) {
    throw new Error("tableHtmlById must include at least one table.");
  }

  const $ = cheerio.load(html, { decodeEntities: false });

  entries.forEach(([tableId, newTableHtml]) => {
    if (!tableId) {
      throw new Error("tableHtmlById is missing a tableId.");
    }
    if (!newTableHtml) {
      throw new Error(`newTableHtml is required for table "${tableId}"`);
    }
...
function normalizeTableEntries(tableHtmlById) {
  if (!tableHtmlById || typeof tableHtmlById !== "object") {
    throw new Error("tableHtmlById must be an object or Map.");
  }

  if (tableHtmlById instanceof Map) {
    return Array.from(tableHtmlById.entries());
  }

  return Object.entries(tableHtmlById);
}
...
async function updateTablesBySlug({ type, slug, tableHtmlById }) {
  const content = await fetchBySlug({ type, slug });
  if (!content) {
    throw new Error(
      `Ghost ${type} with slug "${slug}" not found`,
  ...
  const updatedHtml = replaceTablesHtml({
    html: content.html || "",
    tableHtmlById,
  });
