...
const FILTERABLE_FIGURE_CLASS = "rp-table--filterable";
...
function generateTableHtml(tableSpec) {
  ...
  const { tableClasses, tableAttributes, figureClasses, figureAttributes } =
    buildTableMeta(table, settings);
  const sortableColumns = isTableSortableColumns(tableClasses, settings);
  const sortableRows = isTableSortableRows(tableClasses, settings);
...
function buildTableMeta(table, settings) {
...
function normalizeSettings(settings) {
  ...
  if (normalized.sortableColumns == null) {
    normalized.sortableColumns = true;
  }
  if (normalized.reorder == null) {
    normalized.reorder = "both";
  }
...
function applyTableSettings(
  ...
  if (settings.filters && settings.filters.enabled) {
    figureClasses.add(FILTERABLE_FIGURE_CLASS);
    if (!hasAttr(figureAttributes, "data-filter-columns")) {
      figureAttributes["data-filter-columns"] =
        settings.filters.columns || "list";
    }
    if (!hasAttr(figureAttributes, "data-filter-expand")) {
      figureAttributes["data-filter-expand"] =
        settings.filters.expand || "none";
    }
    if (settings.filters.search && !hasAttr(figureAttributes, "data-filter-search")) {
      figureAttributes["data-filter-search"] = "true";
    }
  }
...
function renderColumnHeader(column) {
  ...
  } else if (!setHasPrefix(classes, "column-align-")) {
    classes.add("column-align-left");
  }
  if (column.valign) {
    classes.add(`column-valign-${column.valign}`);
  } else if (
    !setHasPrefix(classes, "column-valign-") &&
    !setHasPrefix(classes, "cell-valign-")
  ) {
    classes.add("cell-valign-bottom");
  }
  ...
  if (column.filter && column.filter.type) {
    ...
    if (!hasAttr(attributes, "data-filter-type")) {
      attributes["data-filter-type"] = column.filter.type;
    }
...
function renderRow(row, columns, { sortableColumns, sortableRows }) {
  ...
  } else if (!setHasPrefix(classes, "row-valign-")) {
    classes.add("row-valign-middle");
  }
  ...
  if (rowHasLabel) {
    const headerSpec = row.header || {};
    const headerClasses = new Set(headerSpec.classes || []);
    const headerAttributes = { ...(headerSpec.attributes || {}) };

    if (headerSpec.align) {
      headerClasses.add(`cell-align-${headerSpec.align}`);
    } else if (!setHasPrefix(headerClasses, "cell-align-")) {
      headerClasses.add("cell-align-left");
    }
    if (headerSpec.valign) {
      headerClasses.add(`cell-valign-${headerSpec.valign}`);
    }
...
function setHasPrefix(valueSet, prefix) {
  if (!valueSet || valueSet.size === 0) return false;
