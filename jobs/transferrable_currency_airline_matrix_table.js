...
function buildMatrixTable({ sources, transferRows }) {
  const columns = [
    {
      key: "program",
      label: "Program",
...
const job = {
  ...
  fetchData: async (pool) => {
    const airlineSourceIds = SOURCE_PROGRAMS.map((program) => program.id);
    const hotelSourceIds = HOTEL_SOURCE_PROGRAMS.map((program) => program.id);
    const nameLookupIds = Array.from(
      new Set([...airlineSourceIds, ...hotelSourceIds]),
    );
    ...
    const airlineSources = SOURCE_PROGRAMS.map((program) => ({
      id: program.id,
    ...
    const missingHotelSourceNames = HOTEL_SOURCE_PROGRAMS.filter(
      (program) => !sourceNameById.has(program.id),
    );
    if (missingHotelSourceNames.length > 0) {
      throw new Error(
        `Missing LoyaltyProgram short_name for IDs: ${missingHotelSourceNames
          .map((program) => program.id)
          .join(", ")}`,
      );
    }

    const hotelSources = HOTEL_SOURCE_PROGRAMS.map((program) => ({
      id: program.id,
      name: sourceNameById.get(program.id) || program.name,
    })).sort((a, b) =>
      (a.name || "").localeCompare(b.name || "", "en", { sensitivity: "base" }),
    ...
    const [airlineTransferRows] = await pool.query(
      `
        SELECT
    ...
    const [hotelTransferRows] = await pool.query(
      `
        ...
        JOIN LoyaltyProgram lp_to
          ON ctp.to_loyalty_program_id = lp_to.id
        LEFT JOIN Hotelier h
          ON h.loyalty_program_id = lp_to.id
        WHERE ctp.from_loyalty_program_id IN (?)
          AND ctp.is_active = 1
          AND lp_from.is_active = 1
          AND lp_to.is_active = 1
  ...
  tables: [
    {
      ...
      table: TABLE_BASE,
      buildTable: ({ airlineSources, airlineTransferRows }) =>
        buildMatrixTable({
          sources: airlineSources,
          transferRows: airlineTransferRows,
        }),
    },
    {
      id: "transferrable_currency_hotel_matrix_table",
      output: {
        jsonKey: "transferrable_currency_hotel_matrix.json",
        htmlKey: "transferrable_currency_hotel_matrix.html",
      },
      table: TABLE_BASE,
      buildTable: ({ hotelSources, hotelTransferRows }) =>
        buildMatrixTable({
